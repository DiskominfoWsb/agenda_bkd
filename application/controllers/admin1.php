...
//		}
		...
		//upload config 
		$config['upload_path'] 		= './upload/surat_kegiatan';
		$config['allowed_types'] 	= 'gif|jpg|png|pdf|doc|docx';
		$config['max_size']			= '50000';
		$config['max_width']  		= '3000';
		$config['max_height'] 		= '3000';
		...
		if ($mau_ke == "del") {
			...
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
				$this->db->query("INSERT INTO t_kegiatan VALUES (NULL, '$kode','$no_keg','$tahun','$tempat','$pegawai',$isi_keg',','$tgl_kegiatan','$tgl_kegiatan_end','$jam','$jam_end',NOW(),'$unit_pengolah','".$this->session->userdata('admin_id')."')");
			} else {
				$this->db->query("INSERT INTO t_kegiatan VALUES (NULL, '$kode','$no_keg','$tahun','$tempat','$pegawai','$isi_keg','$tgl_kegiatan','$tgl_kegiatan_end','$jam','$jam_end',NOW(), '$unit_pengolah','".$this->session->userdata('admin_id')."')");
			}		
			...
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
				$this->db->query("UPDATE t_kegiatan SET kode = '$kode', no_keg= '$no_keg', tahun = '$tahun', tempat = '$tempat',pegawai= '$pegawai',isi_keg= '$isi_keg',  tgl_keg='$tgl_kegiatan', tgl_keg_end='$tgl_kegiatan_end',jam='$jam',jam_end='$jam_end',unit_pengolah = '$unit_pengolah' WHERE id = '$idp'");
			} else {
				$this->db->query("UPDATE t_kegiatan SET kode = '$kode', no_keg = '$no_keg', tahun = '$tahun', tempat= '$tempat',pegawai= '$pegawai', isi_keg= '$isi_keg',tgl_keg='$tgl_kegiatan', tgl_keg_end='$tgl_kegiatan_end',jam='$jam',jam_end='$jam_end', unit_pengolah = '$unit_pengolah'  WHERE id = '$idp'");
			}	
			...
			$a['data']		= $this->db->query("SELECT * FROM t_kegiatan order by tahun DESC, no_keg DESC LIMIT $awal, $akhir ")->result();
	...
	public function backup() {
     $this->load->dbutil();
     $prefs = array(
         'tables'      => array('ref_klasifikasi', 'tr_instansi', 't_admin', 't_disposisi', 't_instansi', 't_notadinas_keluar', 't_notadinas_masuk', 't_pejabat', 't_surat_keluar', 't_surat_keputusan', 't_surat_masuk', 't_unit_pengolah'),  
         'ignore'      => array(),          
         'format'      => 'txt',           
	...
	public function login() {
		...
		$a['data2']	= $this->db->query("SELECT a.* ,date_format(a.tgl_keg,'%Y-%m-%d') as tgl_keg_, date_format(a.tgl_keg_end,'%d-%m-%Y') as tgl_keg_end_ FROM t_kegiatan a where a.tgl_keg >= CURDATE() order by tgl_keg ASC ")->result();
		$a['data1']	= $this->db->query("SELECT DISTINCT date_format(tgl_keg,'%Y-%m-%d') as tgl_keg1 FROM t_kegiatan where tgl_keg >= CURDATE() order by tgl_keg ASC")->result();
